...
[[ -d /shared/gitian-builder/inputs/bitcoin ]] || \
git clone -b \$1 --depth 1 \$2 /shared/gitian-builder/inputs/bitcoin && \
sudo apt-get --no-install-recommends -yq install \$( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/\"//g;s/- //;p}}' /shared/gitian-builder/inputs/bitcoin/contrib/gitian-descriptors/*|sort|uniq )" > /home/ubuntu/cacheit.sh && \
printf "\
bash /home/ubuntu/cacheit.sh \$1 \$2 && \
cd /shared/gitian-builder; \
./bin/gbuild --commit bitcoin=\$1 --url bitcoin=\$2 \$3" > /home/ubuntu/runit.sh
CMD ["v0.12.1-bitcore-3","https://github.com/bitpay/bitcoin.git","./inputs/bitcoin/contrib/gitian-descriptors/gitian-linux.yml"]
